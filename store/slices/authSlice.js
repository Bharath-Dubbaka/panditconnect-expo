...
export const initAuth = createAsyncThunk("auth/init", async (_, { rejectWithValue }) => {
  try {
    ...
    if (!token || !storedUser) return { token: null, user: null, userType: null };
...
// ── Pandit login ──────────────────────────────────────────
export const loginPandit = createAsyncThunk("auth/loginPandit", async ({ email, password }, { rejectWithValue }) => {
  try {
...
// ── Pandit register ───────────────────────────────────────
export const registerPandit = createAsyncThunk("auth/registerPandit", async (data, { rejectWithValue }) => {
  try {
...
// ── Update local user ─────────────────────────────────────
export const updateUser = createAsyncThunk("auth/updateUser", async (updates, { getState }) => {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    const handlePending = (state) => { state.authLoading = true; state.error = null; };
    ...
    builder
      ...
      .addCase(logout.fulfilled, (state) => {
        state.token = null; state.user = null; state.userType = null; state.error = null;
      })
      .addCase(updateUser.fulfilled, (state, action) => { state.user = action.payload; });
  },
});
